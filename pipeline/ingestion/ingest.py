...
# Initialize chunker - breaks massive pages into precise, searchable pieces
text_splitter = RecursiveCharacterTextSplitter(chunk_size=1000, chunk_overlap=150)

def parse_textbook(file_path: Path) -> list[Document]:
    """Parses D2 textbook into smaller chunks with injected topic/page context."""
    documents = []
    ...
    current_topic = "Introduction" # Tracks the topic across multiple pages
    ...
    for i in range(1, len(pages), 2):
        ...
        # Track the Topic (Looks for patterns like "1.3- SALIENT FEATURES...")
        # Matches digits.digits followed by a hyphen and text
        topic_match = re.search(r'(\d+\.\d+\s*[-–]\s*[^\n]+)', page_text)
        if topic_match:
        ...
        for chunk in chunks:
            # THE MAGIC TRICK: Inject context directly into the text so vector search finds it easily
            enriched_content = (
                f"[Source: {current_chapter} | Topic: {current_topic} | Page: {page_num}]\n"
                f"{chunk}"
            )
    ...
    print(f"[+] Parsed {len(documents)} context-enriched chunks from Textbook ({file_path.name}).")
    return documents

def parse_guidebook(file_path: Path) -> list[Document]:
    """Parses D3 guide into section-level chunks with metadata."""
    documents = []
    if not file_path.exists():
        print(f"[!] Warning: {file_path.name} not found.")
        return documents
    ...
    for i in range(1, len(sections), 2):
        ...
        # Split the guide section into smaller chunks too
        chunks = text_splitter.split_text(section_text)
        
        for chunk in chunks:
            enriched_content = (
                f"[Source: {q_type} | Chapter: {chapter_str}]\n"
                f"{chunk}"
            )
    ...
    print(f"[+] Parsed {len(documents)} context-enriched chunks from Guide ({file_path.name}).")
    return documents
